...
double NeuralNetwork::adapt_learning_rate(double learning_rate) { return learning_rate; }

double NeuralNetwork::adapt_momentum(double momentum) { return momentum; }
	...
	ARRAY_2D avg_bias_derivative, prev_bias_deltas;
	    ...
	    for (int i = 0; i < samples.size(); i += batch_size) {
	    	...
	    	prev_bias_deltas = this->update_bias_weights(avg_bias_derivative, prev_bias_deltas, learning_rate, momentum);
                ...
                learning_rate = adapt_learning_rate(learning_rate);
                momentum = adapt_momentum(momentum);
	    }
        }
}
